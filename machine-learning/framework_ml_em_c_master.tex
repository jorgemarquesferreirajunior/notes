...
\large Do Zero ao Producao: Matematica, Algoritmos e Arquitetura}
\author{Guia Master Avancado}
\date{\today}
...
\begin{itemize}
    \item Conhecimento solido de C (ponteiros, alocacao dinamica, structs)
    \item Matematica basica de graduacao (calculo, algebra linear)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Fundamentos Matematicos e Teoricos do Aprendizado}

\section{O Problema Fundamental do Aprendizado}

O aprendizado de maquina supervisionado pode ser formalizado como um problema de otimizacao. Dado um conjunto de dados $\mathcal{D} = \{(\mathbf{x}_i, y_i)\}_{i=1}^N$ onde $\mathbf{x}_i \in \mathbb{R}^d$ e $y_i \in \mathbb{R}^k$, queremos encontrar uma funcao $f: \mathbb{R}^d \to \mathbb{R}^k$ parametrizada por $\theta$ que minimize:

\[
\theta^* = \arg \min_{\theta} \mathbb{E}_{(\mathbf{x},y) \sim p_{\text{data}}} [\mathcal{L}(f(\mathbf{x};\theta), y)]
\]

Na pratica, como $p_{\text{data}}$ e desconhecida, aproximamos pelo risco empirico:

\[
\theta^* \approx \arg \min_{\theta} \frac{1}{N} \sum_{i=1}^N \mathcal{L}(f(\mathbf{x}_i;\theta), y_i)
\]

\subsection{Espaco de Hipoteses e Capacidade}

Uma rede neural define um espaco de hipoteses $\mathcal{H}$. O teorema da aproximacao universal (Cybenko, 1989) estabelece que uma rede feedforward com uma camada oculta pode aproximar qualquer funcao continua em um compacto de $\mathbb{R}^d$.

\begin{theorem}[Aproximacao Universal]
Seja $\sigma$ uma funcao de ativacao nao constante, limitada e monotona. Entao o conjunto de funcoes da forma:
\[
f(\mathbf{x}) = \sum_{j=1}^m v_j \sigma(\mathbf{w}_j^T\mathbf{x} + b_j)
\]
e denso em $C([0,1]^d)$ no espaco das funcoes continuas.
\end{theorem}

\section{Deducao Detalhada da Funcao de Custo}

\subsection{Mean Squared Error (MSE)}

Para problemas de regressao, assumimos que a variavel alvo e gerada por:
\[
y = f(\mathbf{x};\theta) + \epsilon, \quad \epsilon \sim \mathcal{N}(0, \sigma^2)
\]

A verossimilhanca dos dados e:
\[
...
Ignorando constantes, minimizar MSE e equivalente a maximizar a verossimilhanca sob ruido Gaussiano.

\subsection{Cross-Entropy para Classificacao}

Para classificacao binaria, assumimos $y \in \{0,1\}$ e modelamos:
\[
...
Para classificacao multiclasse com $K$ classes, generalizamos via softmax:
\[
...
\section{Otimizacao e Gradiente Descendente}

\subsection{Deducao do Gradiente}
...
\subsection{Analise de Convergencia}
...
O backpropagation e uma aplicacao eficiente da regra da cadeia para funcoes compostas vetoriais. Seja $f: \mathbb{R}^m \to \mathbb{R}^n$ e $g: \mathbb{R}^n \to \mathbb{R}^p$. Entao:
\[
...
\subsection{Diferenca entre Gradiente e Jacobiano}

Para uma funcao escalar $L: \mathbb{R}^n \to \mathbb{R}$, $\nabla L$ e um vetor. Para uma funcao vetorial $\mathbf{f}: \mathbb{R}^n \to \mathbb{R}^m$, a derivada e uma matriz Jacobiana.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Implementacao Base em C: Estruturas e Gerenciamento de Memoria}

\section{Gerenciamento de Memoria para Tensores}

A base de qualquer framework e o gerenciamento eficiente de arrays multidimensionais. Vamos implementar do zero:
...
typedef struct Tensor {
    float* data;          // Dados contiguos em memoria
    float* grad;          // Gradientes acumulados
    int* shape;           // Dimensoes do tensor (ex: [3,224,224])
    int ndim;             // Numero de dimensoes
    int size;             // Produto das dimensoes
    int refcount;         // Contagem de referencias para garbage collection
    
    // Para autograd (sera expandido depois)
...
\section{Funcoes de Criacao e Destruicao}

\begin{lstlisting}[language=C, caption=Alocacao e liberacao de memoria]
...
\section{Operacoes Basicas Otimizadas}

Operacoes vetorizadas usando loops otimizados:

\begin{lstlisting}[language=C, caption=Operacoes element-wise]
...
Implementacao de multiplicacao de matrizes com tres loops aninhados (O(n³)) - posteriormente otimizaremos com blocagem e SIMD:

\begin{lstlisting}[language=C, caption=Multiplicacao de matrizes]
...
\section{Estrategias de Otimizacao de Memoria}

\subsection{Pooling de Alocacao}
Para evitar fragmentacao de memoria, implementamos um pool de alocacao:

\begin{lstlisting}[language=C, caption=Alocador de memoria customizado]
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Rede Neural: Matematica e Implementacao Detalhada}

\section{Arquitetura de uma Rede Neural}

\subsection{Neuronio Artificial: Modelo Matematico}
...
Onde $\phi$ e a funcao de ativacao. As derivadas sao cruciais para backpropagation:
...
\section{Backpropagation: Deducao Passo a Passo}
...
\subsubsection{Derivadas da Camada de Saida}
\[
...
\subsection{Implementacao Completa do Backprop}
...
void mlp_backward(MLP* net, Tensor* loss_grad) {
    ...
    for(int l = net->n_layers - 1; l >= 0; l--) {
        ...
        // Gradiente dos parametros da camada atual
        ...
        if(l > 0) {
            ...
            // Multiplicar pela derivada da ativacao
...
\section{Inicializacao de Pesos}

A inicializacao correta dos pesos e critica para evitar gradientes desvanecentes/explosivos.

\subsection{Xavier/Glorot Initialization}
Para ativacoes sigmoid/tanh:
\[
...
typedef struct Tensor {
    ...
    char grad_computed;  // Para evitar computacao duplicada
} Tensor;
\end{lstlisting}

\section{Implementacao de Operacoes com Grafo}

\begin{lstlisting}[language=C, caption=Operacao de multiplicacao com autograd]
...
\section{Topological Sort e Propagacao Reversa}

\begin{lstlisting}[language=C, caption=Backward recursivo com ordenacao topologica]
void tensor_backward(Tensor* loss) {
    ...
    // Coleta todos os nos do grafo via DFS
    AutogradNode** nodes = NULL;
    int n_nodes = 0;
    collect_nodes(loss->grad_node, &nodes, &n_nodes);
    
    // Ordenacao topologica (DFS reversa)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Camadas Avancadas e Funcoes de Ativacao}

\section{Camada Linear (Fully Connected)}

\begin{lstlisting}[language=C, caption=Implementacao completa da camada Linear]
...
\section{Funcoes de Ativacao com Autograd}
...
Tensor* relu_forward(Tensor* x) {
    ...
    if(x->requires_grad) {
        ...
        out->grad_node->op_ctx = (void*)x;  // Salva input para mascara
    }
...
\subsection{Softmax Estavel Numericamente}

Softmax pode sofrer overflow/underflow. A versao estavel subtrai o maximo:

\[
p_i = \frac{e^{z_i - \max(z)}}{\sum_j e^{z_j - \max(z)}}
\]

\begin{lstlisting}[language=C, caption=Softmax estavel e seu gradiente]
Tensor* softmax_forward(Tensor* x, int dim) {
    ...
    for(int b = 0; b < batch_size; b++) {
        // Encontra maximo para estabilidade numerica
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Redes Convolucionais: Implementacao Profunda}

\section{Fundamentos da Convolucao Discreta}
...
\subsection{Implementacao Ingênua vs Otimizada}

\begin{lstlisting}[language=C, caption=Convolucao 2D com 6 loops]
...
void conv2d_backward(Conv2D* layer, Tensor* grad_output) {
    ...
    // grad_input: convolucao transposta (full convolution)
...
\section{Otimizacoes: Im2Col e GEMM}

A abordagem de 6 loops e muito lenta. Podemos converter convolucao em multiplicacao de matrizes:

\begin{lstlisting}[language=C, caption=Algoritmo im2col para convolucao otimizada]
Tensor* im2col(Tensor* im, int k_h, int k_w, int stride, int pad) {
    ...
    // Matriz de saida: [batch * out_h * out_w, channels * k_h * k_w]
...
Tensor* conv2d_im2col(Conv2D* layer, Tensor* input) {
    // Converte para multiplicacao de matrizes
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Otimizadores: Teoria e Implementacao}
...
\begin{lstlisting}[language=C, caption=Implementacao completa do Adam]
typedef struct Adam {
    float lr;
    float beta1;
    float beta2;
    ...
    int* t;      // Timestep para cada parametro
    int n_params;
} Adam;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Treinamento e Validacao}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Serializacao e Persistencia de Modelos}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Estudos de Caso e Projetos Praticos}

\section{Exemplo 1: MNIST do Zero}

Implementacao completa de reconhecimento de digitos:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Otimizacoes Avancadas e Performance}

\section{BLAS-Level Optimizations}

\subsection{Tiling para Cache L1/L2}

\begin{lstlisting}[language=C, caption=Multiplicacao de matrizes com tiling]
...
\begin{lstlisting}[language=C, caption=Usando SSE/AVX para operacoes vetoriais]
#include <immintrin.h>
...
\begin{lstlisting}[language=C, caption=Paralelizacao de operacoes]
#include <omp.h>
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Proximos Passos: Alem do Framework}

\section{Suporte a GPU com CUDA}

\begin{lstlisting}[language=C, caption=Interface CUDA para operacoes]
#ifdef USE_CUDA
...
\section{Implementacao de Transformers}
...
\begin{lstlisting}[language=C, caption=Cast de precisao mista]
...
void matmul_mixed(Tensor* a_fp32, Tensor* b_fp32, Tensor* out_fp32) {
    ...
    // Multiplicacao em FP16 (usando hardware nativo)
...
\begin{itemize}
    \item Sistema de tensores com gerenciamento de memoria
    \item Autograd com grafo computacional dinamico
    \item Camadas: Linear, Conv2D, BatchNorm, Dropout
    \item Ativacoes: ReLU, Sigmoid, Tanh, Softmax
    \item Otimizadores: SGD, Adam
    \item Funcoes de custo: MSE, Cross-Entropy
    \item DataLoaders e batching
    \item Serializacao de modelos
    \item Otimizacoes de performance: SIMD, OpenMP, CUDA
\end{itemize}
...
O proximo passo e expandir para arquiteturas modernas como Transformers, GANs, e modelos de difusao, sempre mantendo a filosofia de implementar cada detalhe do zero para compreensao profunda.
